...
def request2(index) :
    ...
    print(Helper1.requestInference('testfile_{}'.format(index),'where is capital of Korea?',0,0)," time : ",time.time_ns())
def request1(index) :
    Helper2 = rtllm("127.0.1.1",10000)
    Helper2.connectToServer()
    Helper2.tryConnectionTest()
    print(Helper2.requestInference('testfile_{}'.format(index),'Explain history of Korea',1,1)," time : ",time.time_ns())
...
# with open("preemption.csv","w+") as my_csv:
