...
def inference(helperIndex,sessionName,input,requestID,priority):
    ...
    reply = HelperList[helperIndex].requestInference(sessionName,input,requestID,priority)
    end = time.time_ns()
    print('requestID : {} - Priority : {} - time : {} - reply : {}'.format(sessionName,priority,(end-start)/1000000,reply))
