...
class GenerationMixin:
    ...
    # ywha edit
    def talk(self,tokenizer,encoded_input,emergency=False,count=50,Cache=None) :
        ...
        if emergency == False : # no emergency mode
            ...
            if Cache == None:
                ...
                # input = [
                #     {"role": "assistant", "content": "You are a chatbot who answers my question in one word"},
                #     {"role": "user", "content": (question)}]

                # encode input using tokenizer      
                # encoded_input = tokenizer.apply_chat_template(input,return_dict=True,return_tensors='pt').to(self.device)
                # generate output using kv Cache
                output = self._my_generate(**encoded_input,
                                           max_new_tokens=count,
                                           emergency=True,
                                           do_sample=True, top_p=0,
    ...
    def _my_generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            if emergency == False:
                result = self._generate_no_emergency(
                    input_ids,
                    logits_processor=prepared_logits_processor,
                    stopping_criteria=prepared_stopping_criteria,
                    generation_config=generation_config,
                ...
                result = self._generate_emergency(
                    input_ids,
                    logits_processor=prepared_logits_processor,
                    stopping_criteria=prepared_stopping_criteria,
                    generation_config=generation_config,
    ...
    def _generate_emergency(
        ...
        **model_kwargs,
        ...
        threshold = torch.tensor(20).to(input_ids.device)
        ...
        while self._has_unfinished_sequences(
            ...
            # start = time.time_ns()
            # print('condition check : ',(start-end)/1000000)
            ...
            # if eot token score over threshold, finish
            # threshold currently set hardcoded
            
            condition = next_token_logits[-1][128009] > threshold
            if condition:
                next_tokens = torch.tensor([128009]).to(input_ids.device)

            # check preemption flag
            # print('check flag : ',self.getPreemptionFlag())
            if self.getPreemptionFlag() == True:
                next_tokens = torch.tensor([128009]).to(input_ids.device)
            ...
            # put next_token to _output_tokens
            if _output_token == None :
                _output_token = next_tokens
            else :
                _output_token = torch.cat((_output_token,next_tokens),dim=-1)
            ...
            # end = time.time_ns()
            # print('inference : ',(end-start)/1000000)
    ...
    def _generate_no_emergency(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # print('check flag : ',self.getPreemptionFlag())
            if self.getPreemptionFlag() == True:
                # next_tokens = torch.cat([next_tokens,torch.tensor([128009]).to(/input_ids.device)])
                stop_tokens = torch.tensor([128009]).to(input_ids.device)
                if has_eos_stopping_criteria:
                    stop_tokens = stop_tokens * unfinished_sequences + pad_token_id * (1 - unfinished_sequences)
                # put next_token to _output_tokens
                if _output_token == None :
                    _output_token = next_tokens
                else :
                ...
                # update generated ids, model inputs, and length for next step
                input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
                input_ids = torch.cat([input_ids, stop_tokens[:, None]], dim=-1)
                if streamer is not None:
                    streamer.put(next_tokens.cpu())
